<%@ page import="com.hupeng.lab1.Person" %>
<%@ page import="java.util.ArrayList" %>
<%@ page import="java.util.List" %>
<%@ page import="com.hupeng.lab1.Dog" %><%--
...
%>
    <ul>
        <c:forEach var="a" items="${words}" >
            <li>${a}</li>
        </c:forEach>
    </ul>
    <ul>
        <c:forEach var="a" items="${words}" step="2" >
            <li>${a}</li>
        </c:forEach>
    </ul>
...
<ul>
    <c:forEach var="person" items="${AllPerson}">
        <li>person name:${person.name} persons' dog name:${person.dog.name}</li>
    </c:forEach>
</ul>
<%
